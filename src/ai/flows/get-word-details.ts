...
For the given German word or phrase "{{wordOrPhrase}}", provide a comprehensive set of details in the specified JSON format. Your entire output, including all descriptions and explanations, MUST be in Russian unless it is a German word itself.

1.  **translation**: Provide the primary Russian translation.
2.  **alternativeTranslations**: Provide a list of up to 3 alternative Russian translations if they exist. If no alternatives, return an empty array.
...
7.  **prepositionDetails**: If it's a preposition:
    ...
    - **dualCaseExplanation**: For 'Wechselpräposition', provide a clear explanation **in Russian**: "Akkusativ (куда?) / Dativ (где?)".
...
8.  **conjunctionDetails**: If it's a conjunction, specify the verb position it leads to ('secondPosition' or 'endOfSentence') and provide a Russian explanation (e.g., "глагол на втором месте", "глагол в конце предложения").
