...
const prompt = ai.definePrompt({
  ...
  {{#ifEquals practiceType "article"}}
  Тип задания: Проверка артикля.
  Ввод пользователя: "{{userInput}}"
  Ожидаемый артикль: "{{expectedAnswer}}"
  {{/ifEquals}}

  {{#ifEquals practiceType "perfect"}}
  ...
  - Проверьте, является ли ввод пользователя ("{{userInput}}") правильным артиклем ("{{expectedAnswer}}") для существительного "{{word}}".
  - isCorrect должен отражать, был ли предоставлен правильный артикль.
  - Если правильно, дайте короткое подтверждение.
  - Если неверно:
    1.  Объясните, почему правильный артикль — "{{expectedAnswer}}".
