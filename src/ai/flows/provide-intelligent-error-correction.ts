...
const prompt = ai.definePrompt({
  ...
  - **Если ошибка только в слове:** "Падеж '{{correctCase}}' вы определили абсолютно верно! Молодец! Однако в самом слове ошибка. Правильно писать '{{expectedAnswer}}'. Объясните ПОЧЕМУ (например, 'потому что 'Park' мужского рода, и в Akkusativ артикль будет 'den')."
  - **Если ошибки в обоих:** "Здесь есть ошибки и в выборе падежа, и в написании слова. Правильный падеж — '{{correctCase}}'. Объясните ПОЧЕМУ. Исходя из этого, правильная форма слова — '{{expectedAnswer}}'. Давайте разберемся по шагам..." (и далее краткое объяснение).
