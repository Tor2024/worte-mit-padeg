...
const prompt = ai.definePrompt({
  ...
  - **Если все верно:** "Блестяще! И падеж '{{correctCase}}', и форма '{{expectedAnswer}}' абсолютно верны. Так держать!"
  - **Если ошибка только в падеже:** "Падеж выбран неверно. Правильный падеж здесь — '{{correctCase}}'. Объясните ПОЧЕМУ (например, 'потому что предлог 'mit' всегда требует Dativ'). А вот слово '{{userInput}}' вы написали правильно для этого (неверного) падежа. Правильная полная фраза была бы '{{expectedAnswer}}'."
  - **Если ошибка только в слове:** "Падеж '{{correctCase}}' вы определили абсолютно верно! Молодец! Однако в самом слове ошибка. Правильно писать '{{expectedAnswer}}'. Объясните ПОЧЕМУ (например, 'потому что 'Park' мужского рода, и в Akkusativ артикль будет 'den'). **Если ошибка в типе артикля (например, 'einen' вместо 'den'), обязательно объясните, почему нужен определенный или неопределенный артикль в этом конкретном предложении.**"
  - **Если ошибки в обоих:** "Здесь есть ошибки и в выборе падежа, и в написании слова. Правильный падеж — '{{correctCase}}'. Объясните ПОЧЕМУ. Исходя из этого, правильная форма слова — '{{expectedAnswer}}'. Давайте разберемся по шагам..." (и далее краткое объяснение, включая тип артикля, если релевантно).
